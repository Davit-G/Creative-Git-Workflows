...
This wouldn't cover all cases, since in some cases we'd just change small values and parameters to small ui elements, so maybe we'd have to manually choose between picking current and incoming changes vs combining them for every changed chunk that is present.

Regardless, having the repository define an entire project declaratively edited by some external app would be very interesting. 
- This means there is no *"code architecture"* to stress about, and if a useful merge conflict strategy is developed for the specific program in question, then this would trivialise merging several ideas into one. Maybe custom CLI tools can be developed for certain programs to handle branch conflicts.
